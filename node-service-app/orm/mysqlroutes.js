...
route.post("/employees",async function(request,response){ 
    ...
    try{ let loginname = request.body.username; 
        ...
        {replacements: {managername: loginname},model:employees,mapToModel: true,type:sequelize.QueryTypes.SELECT} ).then(function(employees){ 
    ...
    try
        { let employeeid = request.body.employeeid; 
            ...
            ).then(function(employees){ response.status(200).send("record updated"); 
        }) 
    } 
    catch(e)
    { 
